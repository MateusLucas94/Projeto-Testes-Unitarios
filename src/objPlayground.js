...
const calculator = (number1, number2) => {
  const resultadoDasOp = {
    sum: Math.floor(number1 + number2),
    mult: Math.floor(number1 * number2),
    div: Math.floor(number1 / number2),
    sub: Math.floor(number1 - number2),
  };
  return resultadoDasOp;
};

const arrayGenerator = (type, object) => {
  let geradorDeResultado;
  if (type === 'keys') {
    geradorDeResultado = Object.keys(object);
  } else if (type === 'values') {
    geradorDeResultado = Object.values(object);
  } else {
    geradorDeResultado = Object.entries(object);
  }
  return geradorDeResultado;
};
